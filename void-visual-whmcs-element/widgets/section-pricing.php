...
class Void_Wbwhmcse_Section_Pricing {
    ...
    const SHORTCODE_BASE = 'void_wbwhmcse_laouts_price';
    ...
    function __construct() {
        ...
        add_action('init', array($this, 'register_price_controls'));
        add_shortcode( self::SHORTCODE_BASE, array($this, 'render_price'));
		
	}
	
	  
    function register_price_controls() {
		...
		vc_map(array(
			...
			"params" => array(
				...
				array(
					"type" => "textarea_html",
					"class" => "",
					"heading" => __("Featured","void_wbwhmcse"),
					"param_name" => "content",
    ...
    function render_price($atts, $content = null ) {
		...
		extract(shortcode_atts(array(
			...
			$buy_btn_css = apply_filters(
				VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
				vc_shortcode_custom_css_class($buy_btn_css, ' '),
				self::SHORTCODE_BASE,
				$atts
			);

			$buy_btn_css_hover = apply_filters(
				VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
				vc_shortcode_custom_css_class($buy_btn_css_hover, ' '),
				self::SHORTCODE_BASE,
				$atts
			);
		...
		<div class="void-pricing-2 pt-type6">
			<div class="pt-header">
				...
				<?php $images = wp_get_attachment_image_src($package_image, 'pricing_image');
					...
					if (!empty($images[0])) {
						?>
						<img src="<?php echo esc_url($images[0]); ?>" alt="<?php echo esc_attr($table_title); ?>">
						<?php
					}
				?>  

				<h3 class="plan-title">
					<?php if($auto_title == 'yes' && $whmcs_url!='' && $packedge_id){ echo'<script language="javascript" src="'.$whmcs_url.'/feeds/productsinfo.php?pid='. $packedge_id .'&get=name"></script>'; } else{  echo $table_title; } ; ?>
				</h3>
				<h4 class="plan-price">
					<?php if($oldprice!=''){echo '<span class="old-price-span">'.$oldprice.'</span>'; } ?> 
					<?php if( $auto_price == 'yes' && $whmcs_url!='' && $packedge_id ) { echo'<script language="javascript" src="'. esc_url($whmcs_url.'/feeds/productsinfo.php?pid='. $packedge_id .'&get=price&billingcycle='. $billingcycle .'&currency='.$currency_number).'"></script>'. $billingafter; } else{echo $price; } ?>
				</h4>
			</div> 	
		...
		<div class="<?php echo $table_style.' '.$pricing_style;  ?>">
			...
			<div class="pricing-box">
				<div class="pricing-box-head">
					<?php 
						$images = wp_get_attachment_image_src( $package_image, 'pricing_image' );
						if ( $images && isset($images[0]) ) : ?>
						<img src="<?php echo esc_url($images[0]); ?>" alt="<?php echo esc_attr($table_title); ?>">
					<?php endif; ?>
					<span class="on_sale title_shop">
						<?php 
							if( $auto_title == 'yes' && $whmcs_url!='' && $packedge_id ){
								echo'<script language="javascript" src="'. $whmcs_url.'/feeds/productsinfo.php?pid='. $packedge_id .'&get=name"></script>'; 
							} else{  
								echo $table_title;
							}
						?>
